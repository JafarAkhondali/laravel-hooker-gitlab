...
namespace jafarakhondali\GitDeploy\Http;
...
class GitDeployController extends Controller
{
	public function gitHook(Request $request)
	{
		...
		$git_cmd = !empty(config('gitdeploy.git_cmd')) ? config('gitdeploy.git_cmd') : 'pull';
		...
		// At this point we're happy everything is OK to execute custom command, lets put Laravel into Maintenance mode.
